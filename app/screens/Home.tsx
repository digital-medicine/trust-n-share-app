...
import {languageTag, translate} from '../utils/localization.ts';
...
export default function HomeScreen() {
  ...
  const [energyBurnedToday, setEnergyBurnedToday] = useState<number | null>(
    null,
  );
  const [activeMinutesToday, setActiveMinutesToday] = useState<number | null>(
    null,
  );
  ...
  const [activeMinutesChartData, setActiveMinutesChartData] = useState<{
    labels: string[];
    datasets: {};
  }>({labels: [], datasets: [{data: []}]});
  ...
  useEffect(() => {
    if (healthData.steps && healthData.steps.length > 0) {
      ...
      const stepsToday = healthData.steps.find(
    ...
    if (healthData.activeMinutes && healthData.activeMinutes.length > 0) {
      ...
      const today = new Date().toISOString().split('T')[0];
      const activeMinutes = healthData.activeMinutes.find(
        entry => entry.date === today,
      )?.value;
      if (activeMinutes) {
      ...
      const data = activeMinutesChartDataTemp.map(entry =>
    ...
    if (healthData.heartRate && healthData.heartRate.length > 0) {
      ...
      const today = new Date().toISOString().split('T')[0];
      const heartRate = healthData.heartRate.find(
        entry => entry.date === today,
      )?.value;
      if (heartRate) {
      ...
      <SafeAreaView>
        ...
        <View style={styles.personalHealthContainer}>
          ...
          {healthLoading ? (
            ...
            <View style={styles.personalHealthGrid}>
              <PersonalHealthItem
                title={translate('home.steps')}
                value={stepsToday}
                iconPack="ionicons"
                icon="footsteps"
              />
              <PersonalHealthItem
                title={translate('home.calories')}
                value={energyBurnedToday}
                iconPack="ionicons"
                icon="flame"
              />
              <PersonalHealthItem
                title={translate('home.active-minutes')}
                value={activeMinutesToday}
                iconPack="fontawesome6"
                icon="person-running"
              />
              <PersonalHealthItem
                title={translate('home.heart-rate')}
        ...
        <View style={styles.statisticsContainer}>
          <Text style={styles.header}>
            {translate('home.statistics-header')}
          </Text>
          {healthLoading ? (
            <Text>{translate('general.loading')}</Text>
          ) : (
            <>
              <View style={styles.chartContainer}>
                <Text style={styles.chartHeader}>
                  {translate('home.steps-this-week')}
                </Text>
              ...
              <View style={styles.chartContainer}>
                <Text style={styles.chartHeader}>
                  {translate('home.active-minutes-this-week')}
                </Text>
                {healthData?.activeMinutes?.some(
              ...
              <View style={styles.chartContainer}>
                <Text style={styles.chartHeader}>
                  {translate('home.heart-rate-this-week')}
                </Text>
  ...
  return (
    <View style={personalHealthItemStyles.outer}>
      <View style={personalHealthItemStyles.inner}>
        ...
        {iconPack === 'fontawesome6' && (
          <FontAwesome6 name={icon} size={30} color="#6c6c6c" />
        )}
